...
for card in cards: # 가진 카드들에 대해서
    if card in hashmap: # 만약 가진 카드가 해쉬맵 목록에 있거나 or 없거나에 따라 분류
        hashmap[card] += 1
    else:
        hashmap[card] = 1

print(' '.join(str(hashmap[card]) if card in hashmap else '0' for card in cards_want))
# [조건 만족시 출력값 if 조건 else 조건 불만족 시 출력 값 for i in data]
# [cards_want의 card들에 대해서, 해시맵에 card가 존재한다면 str(hashmap[card])값을 출력 / 그렇지 않으면 0 츌룍]
